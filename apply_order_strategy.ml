...
let rec fv ast = match ast with
     (Var str) -> str::[]
  |  (App (m,n)) -> (fv m)@(fv n)
  |  (Lambda (x, m)) -> List.filter (fun y -> y <> x )  (fv (m)) (* FV(M)のリストからxに一致していないものだけを取り出す*)
...
(* 適用順序戦略ステップ *)
let rec apply_order_strategy_step ast = match ast with
...
(* メイン関数 *)
let apply_order_strategy ast = 
    let rlt = ref ast
        and count = ref 1 in 
      while (not (isNormal (!rlt))) do 
        rlt := apply_order_strategy_step (!rlt);
...
let test2 = App (Lambda ("x", App (Lambda ("y", App (Var "y", Var "x")), Var "x")), Lambda ("z", Var "z"))
(*問１ 適用順序戦略を実現しなさい．*)
(*問２ 値渡し戦略を実現しなさい．*)
